...
from DataSetViewer import DataaSetViewer
import pandas as pd
import numpy as np
...
class UIPrediction:
    ...
    def create_ui(self):
        ...
        columns = ('Avg. Area Income', 'Avg. Area House Age',
                   'Avg. Area Number of Rooms', 'Avg. Area Number of Bedrooms', 'Area Population', 'Original Price', 'Prediction Price')
        ...
        scroll = Scrollbar(coefficient_detail_label)
        ...
        rmse_label = Label(metric_panel, text="Root Square Error (RMSE):")
        rmse_label.grid(row=2, column=0)
        rmse_entry = Entry(metric_panel, text="", width=20, textvariable=self.mae_value)
        ...
        self.prediction_price_label = DoubleVar()
        prediction_price_entry = Entry(input_prediction_panel, text="", width=40, textvariable=self.prediction_price_label)
        ...
        designedby_label=Label(designedby_panel, text="Design by ME")
    ...
    def do_evaluation (self):
        ...
        for i in range(0, len(self.X_test)):
            values = [self.X_test.iloc[i][0], self.X_test[i][1],
                      self.X_test[i][2], self.X_test[i][3], self.X_test[i][4],
            ...
            self.rmse_value.set(np.sqrt(metrics.mean_absolute_error(self.y_test, predictions)))
        ...
        def do_save_model(self):
            ...
            messagebox.showinfo("infor","expoerted model to disk sucessful!")
            pass

        def do_load_model(self):
            self.lm = FileUtil.loadmodel("housingmodel.zip")
            messagebox.showinfo("infor", "loading model from disk sucessful!")
            pass
